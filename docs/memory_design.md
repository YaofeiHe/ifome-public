...
当前聊天控制台会把两类高价值上传材料保守写入 Profile Memory：

- 简历：保存原文、摘要、结构化网页版和源文件路径。
- 项目说明：保存项目画像、技术栈、亮点、面试切入口和源文件路径。
...
聊天控制台的统一输入框会先把这些附件转成可分析文本，再判断它们是否应该沉淀成稳定画像字段，而不是只作为一次性的临时上下文。
