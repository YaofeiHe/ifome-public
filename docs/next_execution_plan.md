...
当前 macOS VM 里的实际共享挂载路径已经确认是 `/Volumes/My Shared Files/informe/ifome`，因为路径里有空格，后续在 VM 内默认先建立无空格别名 `~/agent_lab/shared/informe`，再进入 `~/agent_lab/shared/informe/ifome-openclaw` 作为 OpenClaw 的默认工作区。
...
- 默认核心执行路径已经切到手写顺序 runtime
- LangGraph 相关代码还保留兼容层，但不再是默认主链路
...
这一轮聊天控制台继续往“长期可用”的方向收口，重点不是单轮问答，而是会话管理。

- 聊天和 Boss 模拟都改成了多会话结构
- 可以创建新对话，也可以切回历史对话继续聊
- 当前对话的输入草稿、模式、限定条目、时间窗口、输出结果都会保存在本地缓存里
- 历史对话不是直接只存整段原文，而是按“当前会话保留完整缓存，非当前会话生成压缩摘要”的策略处理
- 压缩方式参考记忆压缩里的组合思路：
  - 最近几轮保留原文
  - 更早历史先做关键词提取
  - 再压成中期摘要
  - 最后交给 LLM 生成适合历史列表显示的标题和摘要
- 这样重启后不需要把每个旧会话的完整历史都立刻塞回前端当前上下文，能明显降低恢复成本和页面延迟
...
记忆管理和聊天联动这边也补了一条明确能力：

- 如果之前已经上传过简历，可以直接在聊天框里说“请用简历更新项目画像”
- 系统会拿已归档简历快照重新整理项目卡片，而不是要求重复上传

## 最新补充：LLM 时间戳、机器之心标题排序与会话历史布局

这轮针对市场信息刷新和两个前端控制台继续收口。

LLM 通用设置新增了一条硬约束：

- 所有 OpenAI-compatible 文本 LLM 调用都会在进入模型前自动注入 `current_timestamp`
- system prompt 头部也会写入当前时间戳，避免模型在 DDL、近期市场信息、会话摘要里遗漏时间基准
- OCR LLM 也同步带上当前时间戳，但仍保持“只转写、不总结”的任务边界

市场信息刷新链路调整为更接近真实在线流程：
...
- 一次主题里的多轮输入 / 问答存进同一个历史
- 当前选中的历史按时间从第一轮展示到最后一轮
- 页面底部放历史列表，摘要控制在两行内，列表可滚动
- 支持创建新对话、切换历史、删除单个历史和多选删除
- 输入页保持“输入 + 输出”的工作流形态
- 聊天控制台保持社交软件对话框形态
- Boss 回复模拟里，“修改提示词”保留在建议回复下方，回复策略和分析改成可展开区域
...
- 先把 Boss 直聘岗位详情、HR 对话和只读搜索纳入统一输入边界
...
- 自然语言 Boss 搜索指令识别，例如“请搜索boss直聘中有关关键词……”
- 通过 `boss-agent-cli` 的 `boss-mcp` 走只读 MCP 白名单
- 搜索前检查 Boss 登录态，未登录或过期时唤起 Boss 官方扫码登录
- 遇到风控或安全检查时停止搜索，清理旧登录态并要求用户手动完成官方登录/检查
- 关闭 ifome 时通过页眉按钮或 `ifome stop` 清理 Boss 登录态
- `ifome install-boss` 可选安装入口，用于安装 `boss-agent-cli[mcp]` 和 Patchright Chromium
...
- 只读搜索结果可以按公司大卡片和岗位小卡片进入卡片列表
- `boss-agent-cli` 源码不进入 ifome 仓库，只作为运行环境中的可选依赖安装

当前边界：

- 当前只允许搜索、详情、城市和状态读取
- 不允许 Boss 聊天、投递、打招呼、交换联系方式、简历修改或招聘者侧写入操作
- 不尝试绕过验证码、安全检查或平台风控
- 还没有 conversation 的二级抽取 schema
- Boss 搜索后的源信息整理、归档结构和卡片显示还需要继续优化；下一阶段应沿用当前只读流程，在不扩大 Boss 权限的前提下优化“大卡片 / 小卡片”的展示、去重、来源字段和关键词标签
