...
- 当前默认执行路径已经切到手写顺序 Runtime，LangGraph 只保留为可选兼容层和 POC 参考。
...
- “核心手写 Runtime”和“是否使用 Qdrant”是两个正交决策：前者解决执行可控与可观测，后者解决向量检索与过滤能力，两者应组合而不是二选一。
...
- `core/chat/rag.py` 与 `apps/api/src/services.py` 已实现 Query Rewrite、多路检索查询、Prompt 拼装和溯源回答，对齐常见 RAG 在线工作流
