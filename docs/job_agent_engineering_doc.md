...
1. 规则和传统数据库先做第一轮判断
2. 从关键词搜索结果、历史卡片、显式搜索参考、向量召回中补充 prompt 上下文
3. 只有在分类模糊、字段缺失、候选重复项落入模糊区间时才升级到 LLM
4. 如果是“一个集团 / 一个业务线下多个岗位”的输入，先识别合集，再生成 overview 父卡和岗位子卡

这样能避免每次都无差别调 LLM，也更方便解释每一步为什么触发。
...
- 链接输入到提醒创建
- 图片输入 OCR 后抽取
- 图片 / PDF / 文本文件先转文本，再按统一文本入口分类和建卡
- 多岗位合集输入是否生成 overview 父卡和岗位子卡
